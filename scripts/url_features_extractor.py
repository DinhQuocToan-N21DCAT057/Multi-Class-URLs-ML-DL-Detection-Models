...
def timer(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if args and hasattr(args[0], 'exec_time'):
                ...
                logging.info(f"Function '{func.__name__}' took {elapsed_time:.2f} seconds, cumulative exec_time: {args[0].exec_time:.2f} seconds")
            else:
                logging.info(f"Function '{func.__name__}' took {elapsed_time:.2f} seconds (no instance with exec_time found)")
            return result  # Return the original function's result
        return wrapper

except_funcs = ['get_state_and_page', 'global_rank', 'page_rank', 'google_index', 'dns_record',
                'count_internal_redirect', 'count_external_redirect', 'count_internal_error', 'count_external_error']
...
class URL_EXTRACTOR(object):
    ...
    def __init__(self, url, label='Unknown' ,enable_logging=False):
        ...
        if self.soup:
            self.Text = self.soup.get_text().encode('utf-8', errors='replace').decode('utf-8')
        else:
            self.Text = ''
        ...
        self.Title = ''
        self.Null_format = ["", "#", "#nothing", "#doesnotexist", "#null", "#void", "#whatever",
    ...
    def words_raw_extraction(self):
        w_domain = re.split(r"\-|\.|\/|\?|\=|\@|\&|\%|\:|\_", (self.domain or "").lower())
        w_subdomain = re.split(r"\-|\.|\/|\?|\=|\@|\&|\%|\:|\_", (self.subdomain or "").lower())
    ...
    def normalize_url(self, link):
        if not link:
            return None
        # remove //
        link = link.replace('//', '/', 1) if link.startswith(self.hostname + '//') else link
    ...
    def get_hrefs_and_anchors(self):
       ...
       if self.soup:
            for href in self.soup.find_all('a', href=True):
                ...
                if self.hostname in href['href'] or self.domain in href['href'] or len(dots) == 1 or not href['href'].startswith('http'):
    ...
    def get_links(self):
        ...
        if self.soup:
            for link in self.soup.find_all('link', href=True):
                ...
                if self.hostname in link['href'] or self.domain in link['href'] or len(dots) == 1 or not link['href'].startswith('http'):
    ...
    def get_scripts(self):
        ...
        if self.soup:
            for script in self.soup.find_all('script', src=True):
                ...
                if self.hostname in script['src'] or self.domain in script['src'] or len(dots) == 1 or not script['src'].startswith(
                        'http'):
    ...
    def get_media_imgs(self):
        ...
        if self.soup:
            for img in self.soup.find_all('img', src=True):
                ...
                if self.hostname in img['src'] or self.domain in img['src'] or len(dots) == 1 or not img['src'].startswith('http'):
    ...
    def get_media_audios(self):
        ...
        if self.soup:
            for audio in self.soup.find_all('audio', src=True):
                ...
                if self.hostname in audio['src'] or self.domain in audio['src'] or len(dots) == 1 or not audio['src'].startswith('http'):
    ...
    def get_media_embeds(self):
        ...
        if self.soup:
            for embed in self.soup.find_all('embed', src=True):
                ...
                if self.hostname in embed['src'] or self.domain in embed['src'] or len(dots) == 1 or not embed['src'].startswith('http'):
    ...
    def get_media_iframes(self):
        ...
        if self.soup:
            for i_frame in self.soup.find_all('iframe', src=True):
                ...
                if self.hostname in i_frame['src'] or self.domain in i_frame['src'] or len(dots) == 1 or not i_frame['src'].startswith(
                        'http'):
                    if not i_frame['src'].startswith('http'):
                        if not i_frame['src'].startswith('/'):
                            self.Media['internals'].append(self.hostname + '/' + i_frame['src'])
    ...
    def get_forms(self):
        ...
        if self.soup:
            for form in self.soup.find_all('form', action=True):
                ...
                if self.hostname in form['action'] or self.domain in form['action'] or len(dots) == 1 or not form['action'].startswith(
                        'http'):
                    if not form['action'].startswith('http'):
                        if not form['action'].startswith('/'):
                            self.Form['internals'].append(self.hostname + '/' + form['action'])
                        elif form['action'] in self.Null_format or form['action'] == 'about:blank':
        ...
        if self.soup:
            for style in self.soup.find_all('style', type='text/css'):
                try:
                    ...
                    if self.hostname in css or self.domain in css or len(dots) == 1 or not css.startswith('http'):
        ...
        if self.soup:
           for head in self.soup.find_all('head'):
                for head_link in self.soup.find_all('link', href=True):
                    ...
                    if self.hostname in head_link['href'] or len(dots) == 1 or self.domain in head_link['href'] or not head_link[
                        'href'].startswith('http'):
                        if not head_link['href'].startswith('http'):
                            if not head_link['href'].startswith('/'):
                                self.Favicon['internals'].append(self.hostname + '/' + head_link['href'])
                ...
                for head_link in self.soup.find_all('link', {'href': True, 'rel': True}):
                    isicon = False
    ...
    def get_iframes(self):
        ...
        if self.soup:
            for i_frame in self.soup.find_all('iframe', width=True, height=True, frameborder=True):
                if i_frame['width'] == "0" and i_frame['height'] == "0" and i_frame['frameborder'] == "0":
                    self.IFrame['invisible'].append(i_frame)
                else:
                    self.IFrame['visible'].append(i_frame)
            for i_frame in self.soup.find_all('iframe', width=True, height=True, border=True):
                if i_frame['width'] == "0" and i_frame['height'] == "0" and i_frame['border'] == "0":
                    self.IFrame['invisible'].append(i_frame)
                else:
                    self.IFrame['visible'].append(i_frame)
            for i_frame in self.soup.find_all('iframe', width=True, height=True, style=True):
    ...
    def tld_in_bad_position(self):
        if self.tld_in_path(self.tld, self.path) == 1 or self.tld_in_subdomain(self.tld, self.subdomain) == 1:
            return 1
        return 0
    ...
    def has_statistical_report(self):
        ...
        try:
            ...
            ip_match = re.search(
                ...
                ip_address)
            if url_match or ip_match: return 1
            elif ip_address == '125.235.4.59': return 2
            else: return 0
    ...
    def ratio_internal_hyperlinks(self):
        ...
        if total == 0: return 0
        else: return self.count_internal_hyperlinks() / total
    ...
    def ratio_external_hyperlinks(self):
        total = self.count_hyperlinks()
        if total == 0: return 0
        else: self.count_external_hyperlinks() / total
    ...
    def count_null_hyperlinks(self):
        return len(self.Href['null']) + len(self.Link['null']) + len(self.Media['null']) + len(self.Form['null']) + len(self.CSS['null']) + len(self.Favicon['null'])

    @timer
    def ratio_null_hyperlinks(self):
        total = self.count_hyperlinks()
        if total == 0: return 0
        else: self.nb_null_hyperlinks() / total
    ...
    def count_internal_error(self):
        ...
        for link in self.Href['internals']:
            try:
                link = self.normalize_url(link)
                if requests.get(link, headers=self.headers, timeout=5).status_code >=400:
                    count+=1
            except:
                continue
        for link in self.Link['internals']:
    ...
    def has_domain_with_copyright(self):
        try:
            m = re.search(u'(\N{COPYRIGHT SIGN}|\N{TRADE MARK SIGN}|\N{REGISTERED SIGN})', self.Text)
    ...
    def page_rank(self):
        ...
        try:
            request = requests.get(url, headers={'API-OPR':self.OPR_API_key})
            result = request.json()
